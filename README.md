...
general ones first, then subject by subject.
...
- [Architecture](AGENTS.md#architecture)
  - [Each context splits its domain from its infrastructure](AGENTS.md#each-context-splits-its-domain-from-its-infrastructure)
  - [Routers own HTTP and nothing else](AGENTS.md#routers-own-http-and-nothing-else)
  - [Everything a context needs lives in it](AGENTS.md#everything-a-context-needs-lives-in-it)
- [Integration — between apps, and with the admin console](AGENTS.md#integration--between-apps-and-with-the-admin-console)
  - [An app declares every surface it contributes](AGENTS.md#an-app-declares-every-surface-it-contributes)
  ...
  - [`emit` records a fact and does only that](AGENTS.md#emit-records-a-fact-and-does-only-that)
  ...
  - [A contribution is pulled, and a failing contributor is skipped](AGENTS.md#a-contribution-is-pulled-and-a-failing-contributor-is-skipped)
  - [Sign-up is a chain of durable reactions](AGENTS.md#sign-up-is-a-chain-of-durable-reactions)
  - [The dashboard collects one card per app](AGENTS.md#the-dashboard-collects-one-card-per-app)
  - [Import downward, event upward](AGENTS.md#import-downward-event-upward)
- [Observability](AGENTS.md#observability)
  - [The journal records what changed the domain](AGENTS.md#the-journal-records-what-changed-the-domain)
  - [The log sink traces the machinery off the request's path](AGENTS.md#the-log-sink-traces-the-machinery-off-the-requests-path)
  - [A line says what no other record says](AGENTS.md#a-line-says-what-no-other-record-says)
  - [Nothing escapes the log chain](AGENTS.md#nothing-escapes-the-log-chain)
  - [A bug is an issue with a lifecycle](AGENTS.md#a-bug-is-an-issue-with-a-lifecycle)
  - [A broken dependency is a bug, a refusal is not](AGENTS.md#a-broken-dependency-is-a-bug-a-refusal-is-not)
  - [A failure that repeats is one bug](AGENTS.md#a-failure-that-repeats-is-one-bug)
  - [The Timeline reads the journal, the log sink and the issues](AGENTS.md#the-timeline-reads-the-journal-the-log-sink-and-the-issues)
  - [Load metrics belong to their app alone](AGENTS.md#load-metrics-belong-to-their-app-alone)
- [Conventions](AGENTS.md#conventions)
  - [Three sessions, and RLS by default](AGENTS.md#three-sessions-and-rls-by-default)
  - [A GET never delivers a session](AGENTS.md#a-get-never-delivers-a-session)
  - [Deferred work rides a durable Postgres queue](AGENTS.md#deferred-work-rides-a-durable-postgres-queue)
  - [CSRF needs no token, and the rate limiter fails open](AGENTS.md#csrf-needs-no-token-and-the-rate-limiter-fails-open)
  - [One set of helpers branches JSON, fragment and page](AGENTS.md#one-set-of-helpers-branches-json-fragment-and-page)
  - [A form is JSON at the door](AGENTS.md#a-form-is-json-at-the-door)
  - [A page's context is assembled from slices its apps own](AGENTS.md#a-pages-context-is-assembled-from-slices-its-apps-own)
  - [`clock.now()` is the only clock](AGENTS.md#clocknow-is-the-only-clock)
  - [Every key is a UUIDv7, every token a UUIDv4](AGENTS.md#every-key-is-a-uuidv7-every-token-a-uuidv4)
  - [daisyUI components, never re-spelled utility chains](AGENTS.md#daisyui-components-never-re-spelled-utility-chains)
  - [Each scenario runs isolated, on both drivers](AGENTS.md#each-scenario-runs-isolated-on-both-drivers)
  - [Assert the settled DOM, never wait on time](AGENTS.md#assert-the-settled-dom-never-wait-on-time)
