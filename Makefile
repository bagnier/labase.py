...
# The single-file-type linters run via `uvx` (pinned, isolated) rather than as project
# dev-deps, so they stay out of pyproject.toml / uv.lock and add no resolution weight.
lint:
	uv run ruff check .
