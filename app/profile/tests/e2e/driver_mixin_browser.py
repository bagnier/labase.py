...
class ProfileBrowserMixin(BrowserBase):
    ...
    def view_profile(self) -> None:
        self._last_response = self._page.goto(self._profile_url(), wait_until="load")

    def update_handle(self, name: str) -> None:
        self._page.goto(self._profile_url(), wait_until="load")
        self._page.fill("input[name=handle]", name)
        with self._page.expect_response(
            lambda r: "/profile" in r.url and r.request.method == "POST"
        ) as resp_info:
            self._page.click("form:has(input[name=handle]) button[type=submit]")
        self._last_response = resp_info.value

    def assert_handle(self, name: str | None) -> None:
        self._page.goto(self._profile_url(), wait_until="load")
        value = self._page.locator("input[name=handle]").input_value()
        if name:
    ...
    def assert_email_read_only(self) -> None:
        self._page.goto(self._profile_url(), wait_until="load")
        disabled = self._page.locator("input[disabled]")
    ...
    def assert_link_to_org_dashboard(self) -> None:
        assert self._page.query_selector("a[href*='/dashboard']") is not None, (
