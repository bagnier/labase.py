...
class TodoApiMixin(ApiBase):
    ...
    def _todo_id_by_title(self, title: str) -> str:
        resp = self._json("GET", self._todos_url())
        todos = resp.json()
        for t in todos:
            if t["title"] == title:
                return t["id"]
    ...
    def have_todo_items(self, titles: list[str]) -> None:
        for title in reversed(titles):
            self._json("POST", self._todos_url(), json={"title": title})

    def view_todo_list(self) -> None:
        self._response = self._run(self._c.get(self._todos_url()))

    def add_todo(self, title: str) -> None:
        self._response = self._json("POST", self._todos_url(), json={"title": title})

    def mark_todo_done(self, title: str) -> None:
        todo_id = self._todo_id_by_title(title)
        self._response = self._json("PATCH", self._todos_url(f"/{todo_id}"), json={"done": True})

    def mark_todo_not_done(self, title: str) -> None:
        todo_id = self._todo_id_by_title(title)
        self._response = self._json("PATCH", self._todos_url(f"/{todo_id}"), json={"done": False})
    ...
    def delete_todo(self, title: str) -> None:
        todo_id = self._todo_id_by_title(title)
        self._response = self._json("DELETE", self._todos_url(f"/{todo_id}"))
