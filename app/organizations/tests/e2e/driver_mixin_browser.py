...
class OrgBrowserMixin(BrowserBase):
    ...
    def _fetch_orgs_for(self, email: str) -> list[dict]:
        ...
        if email == acting:
            return self._read_org_cards_from_profile(self._page)
    ...
    def _active_slug(self) -> str:
        ...
        if slug:
            return slug
        orgs = self._read_org_cards_from_profile(self._page)
    ...
    def assert_is_owner(self) -> None:
        ...
        self._page.goto(f"{self._base_url}/{slug}/members", wait_until="load")
        el = self._page.query_selector(f"[data-member-email='{email}'][data-member-role='owner']")
    ...
    def join_org_as_member(self, org_name: str, email: str) -> None:
        ...
        owner_ctx = self._browser.new_context()
    ...
    def view_org_list(self) -> None:
        self._org_list_response = self._read_org_cards_from_profile(self._page)  # type: ignore[attr-defined]

    def assert_org_in_list(self, org_name: str) -> None:
        org_list = getattr(self, "_org_list_response", None)
        if org_list is None:
            org_list = self._read_org_cards_from_profile(self._page)
        names = [o["name"] for o in org_list]
        assert org_name in names, f"Expected {org_name!r} in org list: {names}"

    def assert_org_absent(self, org_name: str) -> None:
        names = [o["name"] for o in self._read_org_cards_from_profile(self._page)]
    ...
    def assert_workspace_card(self, org_name: str) -> None:
        self._page.goto(f"{self._base_url}/profile", wait_until="load")
        assert self._page.query_selector(f'[data-organisation-card="{org_name}"]') is not None, (
    ...
    def view_org_dashboard(self) -> None:
        ...
        self._last_response = self._page.goto(
            f"{self._base_url}/{slug}/dashboard", wait_until="load"
        )
    ...
    def visit_org_dashboard_unauthenticated(self) -> None:
        self._last_response = self._page.goto(
            f"{self._base_url}/any-org/dashboard", wait_until="load"
        )
