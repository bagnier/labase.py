...
class OrgFileApiMixin(ApiProtocol):
    # populated by sign_in_within_org (via auth mixin delegation)
    _primary_email: str
    ...
    def _reset_multi_user_state(self) -> None:
        # Persist known emails so sign_in_within_org can clean them up
    ...
    def _list_files_with(self, client: httpx.AsyncClient) -> list[dict]:
        resp = self._run(client.get("/files", headers={"accept": "application/json"}))
        assert resp.status_code == 200, f"list_files got {resp.status_code}: {resp.text}"
        return resp.json()

    def _list_files(self) -> list[dict]:
        return self._list_files_with(self._c)
    ...
    def sign_in_within_org(self, email: str, org_name: str) -> None:
        ...
        # Clean slate: delete all known test users (cascades files/memberships/orgs) then re-register
        known = getattr(self, "_known_test_emails", set()) | {email}
        for e in known:
            self._delete_supabase_user(e)
        ...
        if resp.status_code == 200 and resp.json():
            ...
            # Demote alice to member so scenarios about non-admin users work correctly
    ...
    def view_file_list(self) -> None:
        ...
        self._last_file_list: list[dict] | None = None  # reset: primary list is re-fetched live
    ...
    def delete_file(self, filename: str) -> None:
        ...
        if file_id is None:
            ...
            return
        self._response = self._run(
            self._c.delete(f"/files/{file_id}", headers={"accept": "application/json"})
        )
    ...
    def add_member_to_org(self, email: str) -> None:
        self._ensure_multi_user()
        # ensure secondary client exists (registers the user)
        ...
        # Point secondary client at primary org
        client.cookies.set("active_org_id", org_id, domain="testserver")
    ...
    def generate_share_link(self, filename: str) -> None:
        ...
        file_id = self._file_id_by_name(filename)
        resp = self._run(
            self._c.post(f"/files/{file_id}/share", headers={"accept": "application/json"})
        )
    ...
    def view_file_list_as(self, email: str) -> None:
        client = self._client_for(email)
        self._response = self._run(client.get("/files", headers={"accept": "application/json"}))
    ...
    def _current_file_list(self) -> list[dict]:
        ...
        if lfl is not None:
            self._last_file_list = None  # consume once
            return lfl
