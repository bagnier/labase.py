...
- [ ] **[PARTIEL · 🟡 basse] Business-event fire-and-forget : référence tenue, drain manquant** — la task détachée était créée sans référence (CPython pouvait la GC en vol, d'où des trous sporadiques dans le trail). Corrigé 2026-08-15 : `self._detached` + `add_done_callback`, le motif des quatre `create_task` sœurs. Reste le second volet du fix d'origine : **rien ne draine ces tasks à l'arrêt**, donc un SIGTERM pendant un write détaché le perd encore. `apps/shared/events/bus.py` (ex-`observability/business_events.py`).
