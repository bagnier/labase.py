...
-- the event listener and escaped a test's rolled-back transaction. Instead, the same AFTER INSERT
-- trigger that seeds the profile now also writes the ``UserCreated`` fact onto the journal — atomic
...
-- welcome seeders) run off the journal unchanged.
--
-- The event goes to *this schema's* business_events only (no cross-schema write): the journal has no
-- unique key, so a dual-write would duplicate. Each schema's cloned trigger (see provision_schema)
-- writes its own record exactly once. ``user_id`` is an unconstrained reference (no FK) — the
-- journal
