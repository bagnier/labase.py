...
--
--   1. `consumed` re-keys on the business_events row id (a stable bigint the tailer dispatches from),
--      replacing the generated uuid the emit-time outbox used. The ledger is disposable — a dropped
--      row costs at most one redundant re-delivery — so recreate it rather than migrate values.
...
create table public.consumed (
  topic       text        not null,
  event_id    bigint      not null,   -- the business_events.id the consumer processed
