-- Evolve the audit_logs table into the unified business-events journal. Same column shape
-- (kind + user/org + ip/request_id + level + payload), but the producer becomes the typed
-- event bus (apps/shared/events.py) and the store becomes member-readable: RLS scopes it to
...
-- without shared having to map app→icon (a foundation must not name features). Nullable: facts
...
-- authenticated — the journal stays append-only, written only by the admin session.
