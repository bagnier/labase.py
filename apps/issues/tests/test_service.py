...
from apps.issues.domain.service import fingerprint, status_after_occurrence, title_for
...
def test_resolved_issue_regresses_on_another_version():
    status = status_after_occurrence(IssueStatus.resolved, "abc123", "def456")
    assert status is IssueStatus.regressed


def test_resolved_issue_tolerates_occurrences_from_the_fix_version():
    status = status_after_occurrence(IssueStatus.resolved, "abc123", "abc123")
    assert status is IssueStatus.resolved


def test_ignored_and_triaged_statuses_are_sticky():
    assert status_after_occurrence(IssueStatus.ignored, None, "v2") is IssueStatus.ignored
    assert status_after_occurrence(IssueStatus.new, None, "v2") is IssueStatus.new
    assert status_after_occurrence(IssueStatus.unresolved, None, "v2") is IssueStatus.unresolved
