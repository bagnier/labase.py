...
def test_resolves_the_org_scoped_page_for_apps_with_a_detail_route():
    ...
    eid = uuid.uuid7()
    assert entity_url("pages.created", eid, "acme") == f"/acme/pages/by-id/{eid}"
    assert entity_url("calendar.event_created", eid, "acme") == f"/acme/calendar/{eid}"
...
def test_resolves_a_list_anchor_for_apps_with_only_a_list_page():
    ...
    eid = uuid.uuid7()
    assert entity_url("todo.created", eid, "acme") == f"/acme/todos#todo-{eid}"
    assert entity_url("files.uploaded", eid, "acme") == f"/acme/files#file-{eid}"


def test_no_link_when_app_has_no_detail_route_or_data_is_missing():
    eid = uuid.uuid7()
    assert entity_url("learning.card_reviewed", eid, "acme") is None  # app has no route at all
    assert entity_url("pages.created", None, "acme") is None  # no entity
    assert entity_url("pages.created", eid, None) is None  # org handle unknown (cross-org)
