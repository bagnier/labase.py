"""Derive Supabase Studio deep links from ``SUPABASE_URL``.

The console surfaces "open in Supabase" links for advanced management. There is no dedicated
config for the Studio URL, so it is inferred from the project's ``SUPABASE_URL``:
...
def studio_base_url(supabase_url: str) -> str:
    """Map a ``SUPABASE_URL`` to its Studio base URL (no trailing slash)."""
...
def studio_link(supabase_url: str, path: str) -> str:
    """Full Studio URL for a relative ``path`` fragment (e.g. ``auth/users``)."""
    return f"{studio_base_url(supabase_url)}/{path.lstrip('/')}"
