...
write path sits on). Durable async consumers register via ``bus.on`` and are delivered off the log
by the ``listener`` after commit. Reading the trail back for a surface — the humanized activity
feed and contribution calendar — is pure presentation, in ``timeline``.
...
importing a type must stay cheap. The bus, repository, store, timeline and listener are reached at
