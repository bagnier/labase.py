...
write path and timeline projection sit on). Durable async consumers register via ``bus.on`` and
are delivered off the log by the ``listener`` after commit.
...
importing a type must stay cheap. The bus, repository, store and listener are reached at their
submodule paths (``apps.shared.events.bus`` / ``.repository`` / ``.store`` / ``.listener``).
"""
