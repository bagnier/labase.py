"""The one chain every line enters the firehose through — ours and the libraries'.
...
tests hold the seam at :func:`setup_logging` — what a caller logs is what the firehose
...
from apps.shared.observability.firehose import read_firehose
...
def test_a_library_log_reaches_the_firehose(log_chain):
...
def test_an_exception_escaping_a_thread_reaches_the_firehose(log_chain):
...
def test_a_crash_on_the_way_out_is_on_disk_before_the_process_dies(log_chain):
    """Nothing will drain the queue after this line: the writer task is already gone."""
    sys.excepthook(ValueError, ValueError("the process gives up"), None)
    assert [(line.name, line.level) for line in read_firehose()] == [("process.crashed", "error")]


@pytest.mark.asyncio
async def test_an_exception_escaping_a_background_task_reaches_the_firehose(log_chain):
...
def test_an_exception_in_a_destructor_reaches_the_firehose(log_chain):
...
def test_a_warning_raised_by_python_itself_reaches_the_firehose(log_chain):
